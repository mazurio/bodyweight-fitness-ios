...
extension CGImageSourceRef {
    ...
    func GIFPropertiesAtIndex(index: Int) -> GIFProperties? {
        if let properties = CGImageSourceCopyPropertiesAtIndex(self, index, nil) {
            return (properties as Dictionary)[String(kCGImagePropertyGIFDictionary)] as? GIFProperties
        } else {
            return .None
        }
    }
